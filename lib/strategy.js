...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (obj.payload.m && req.method !== obj.payload.htm) {
    return this.fail(this._challenge('invalid_token', 'HTTP method does not signature'));
  }
  ...
  function verified(err, user, key, info) {
    ...
    if (!ok) { return self.fail(self._challenge('invalid_token', 'Invalid signature')); }
...
Strategy.prototype._challenge = function(code, desc, uri) {
  ...
  if (this._realm) {
    params.push('realm="' +this._realm + '"');
  }
  if (this._scope) {
    params.push('scope="' + this._scope.join(' ') + '"');
  }
  if (code) {
  ...
  if (uri && uri.length) {
    params.push('error_uri="' + uri + '"');
  }
  
  return 'PoP' + (params.length ? ' ' + params.join(', ') : '');
};
